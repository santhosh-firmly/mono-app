...
export async function saveSessionToR2(platform, sessionId, events) {
	...
	await platform.env.SESSION_RECORDINGS.put(key, JSON.stringify(events));
}

export async function getSessionFromR2(platform, sessionId) {
	const key = `${sessionId}.json`;
	const object = await platform.env.SESSION_RECORDINGS.get(key);
...
export async function saveSessionMetadata(platform, metadata) {
	...
	await platform.env.SESSION_METADATA.put(sessionId, JSON.stringify(metadata));

	const listKey = 'session_list';
	const existingList = await platform.env.SESSION_METADATA.get(listKey);
	...
	await platform.env.SESSION_METADATA.put(listKey, JSON.stringify(sessionList));
}

export async function getSessionMetadata(platform, sessionId) {
	const data = await platform.env.SESSION_METADATA.get(sessionId);
	return data ? JSON.parse(data) : null;
}

export async function listSessionMetadata(platform, limit = 50, offset = 0) {
	const listKey = 'session_list';
	const data = await platform.env.SESSION_METADATA.get(listKey);
