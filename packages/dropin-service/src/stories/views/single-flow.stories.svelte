<script module>
	...
	import MOCKED_CART from '../assets/cart.json';
	...
	const { Story } = defineMeta({
		title: 'Views/SingleFlow',
		component: SingleFlowView,
		parameters: {
			layout: 'fullscreen'
		}
	});
...
<script>
	let dataWithoutShippingMethod = $state({
		pending: {},
		cart: {
			...MOCKED_CART,
...
<Story name="Checkout started with loading state">
	<SingleFlowView
		data={{ pending: { cart: true } }}
		dispatch={(action, payload) => handleDispatch(dataWithoutShippingMethod, action, payload)}
	/>
</Story>

<Story name="Checkout started without pre-filled data">
	<SingleFlowView
		data={dataWithoutShippingMethod}
		dispatch={(action, payload) => handleDispatch(dataWithoutShippingMethod, action, payload)}
	/>
</Story>

<Story name="Checkout started with pre-filled data">
	<SingleFlowView
		data={preFilledData}
		dispatch={(action, payload) => handleDispatch(preFilledData, action, payload)}
	/>
</Story>

<Story name="Checkout started with pre-filled data and saved address">
	<SingleFlowView {data} dispatch={(action, payload) => handleDispatch(data, action, payload)} />
</Story>

<Story name="Checkout started with pre-filled data and saved address and saved card">
	<SingleFlowView {data} dispatch={(action, payload) => handleDispatch(data, action, payload)} />
</Story>
