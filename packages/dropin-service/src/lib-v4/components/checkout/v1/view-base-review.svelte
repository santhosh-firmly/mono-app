<script>
	...
	import { getShippingDescription } from '$lib-v4/browser/cart-helper.js';
	import {
		ReviewDelivery,
		ReviewPayment,
	...
	import { cViewReview, sWizardLast, sWizardShipping } from '$lib-v4/browser/wizard.js';
	import { sCart, cartUpdateDelivery } from '$lib-v4/browser/api-manager.js';
	import PhoneIcon from '$lib-v4/components/common/svg/phone-icon.svelte';
	import { slide } from 'svelte/transition';
	import { expoOut } from 'svelte/easing';
	import HomeIcon from '$lib-v4/components/common/svg/home-icon.svelte';
	import { sIsNavPlaceOrder } from '$lib-v4/browser/storage.js';
	import { onDestroy, onMount } from 'svelte';
	import SectionHeader from '$lib-v4/components/common/section-header.svelte';
	import EmailIcon from '$lib-v4/components/common/svg/email-icon.svelte';
...
<div {...$$restProps} fobs>
	...
	<div class="flex flex-col gap-2 px-4">
		<div class="flex content-center text-xs">
			<div class="flex basis-8 items-center justify-start"><HomeIcon /></div>
			<div class="">{getShippingAddress()}</div>
		</div>
		{#if $sCart.shipping_info && $sCart.shipping_info.email}
			<div class="flex content-center text-xs">
				<div class="flex basis-8 items-center justify-start"><EmailIcon /></div>
				<div class="">{$sCart.shipping_info.email}</div>
			</div>
		{/if}
		{#if $sCart.shipping_info && $sCart.shipping_info.phone}
			<div class="flex content-center text-xs">
				<div class="flex basis-8 items-center justify-start"><PhoneIcon /></div>
	...
	<div class="text-secondary mt-5 mb-5 flex flex-col gap-1 px-4 text-center text-xs">
		<span>
