<script lang="ts">
	// @ts-nocheck
	import { sCartHive, sDynamoConfig } from '$lib-v4/browser/storage';
...
<div
	...
	class="top-box sticky top-0 bg-white"
...
>
	<div class="container">
		<a
			...
			href="/mk"
			aria-label="Firmly"
			class="header-logo flex h-[64px] flex-col justify-center"
		>
		...
		{#if !logoOnly}
			<nav class="header-main" aria-label="Main">
				...
				<div class="header-button-container">
					...
					{#if showCartButton}
						<button
							class="header-icon-link header-subnav-link header-right-subnav relative"
								...
								{#if showBadgeWhenZero || itemsTotalQuantity > 0}
									{#if badgeStyle === BadgeType.LabelStyle}
										<div class="badge relative">
											<p class="badge-text absolute top-0 text-white">
									...
									{:else}
										<div
											class="circle-badge absolute top-0 right-0 inline-flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-red-500 text-xs text-white dark:border-gray-900"
										>
											{itemsTotalQuantity}
										</div>
									{/if}
