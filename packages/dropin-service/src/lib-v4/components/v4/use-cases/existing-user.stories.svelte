...
<Story
	name="Existing user already logged in"
	...
	parameters={{
		msw: {
			handlers: [
				...
				http.post('*/session/create-otp', async (req, res, ctx) => res(ctx.status(404))),
				http.post('*/cart/shipping-info', async (req, res, ctx) =>
					res(ctx.status(200), ctx.json(cartDataLoggedInUserWithShipping))
				),
				http.post('*/session/join', async (req, res, ctx) => res(ctx.status(500))),
				http.post('*/cart/complete-order-with-saved-payment', async (req, res, ctx) =>
...
<Story
	name="Existing user logs in on place order"
	play={async ({ canvasElement, step }) => {
		const canvas = within(canvasElement);
		const user = userEvent.setup();
		...
		await step('Click on Place Order', async () => {
			const placeOrderButton = canvas.getByText(/Place Order/i);
			await waitFor(() => expect(placeOrderButton).not.toBeDisabled(), {
				timeout: 5000
			});
		...
		await step('Enter OTP', async () => {
			await waitFor(() => canvas.getByTestId('otp-field-0'), {
				timeout: 5000
			});
			const otpField0 = canvas.getByTestId('otp-field-0');
			await waitFor(() => expect(document.activeElement).toBe(otpField0));
			await userEvent.paste('123456');
		});

		await step('Place Order', async () => {
			const otpField0 = canvas.getByTestId('otp-field-0');
			await waitFor(() => expect(otpField0).not.toBeInTheDocument(), {
				timeout: 5000
			});
	...
	parameters={{
		msw: {
			handlers: [
				...
				http.post('*/cart/shipping-info', async (req, res, ctx) =>
					res(ctx.status(200), ctx.json(forceNotLoggedIn(cartDataLoggedInUserWithShipping)))
				),
				http.post('*/session/join', async (req, res, ctx) => res(ctx.status(500))),
				http.post('*/cart/complete-order-with-saved-payment', async (req, res, ctx) =>
					res(ctx.status(200), ctx.delay(1000), ctx.json(forceNotLoggedIn(orderDataLoggedInUser)))
				)
			]
		}
	}}
>
	<div class="theme-provider adoreme-dark">
		<FlowSinglePage cart={cartNotLoggedInUser} {isOrderPlaced} />
	</div>
</Story>
