...
<Group>
	<div class="col-span-2 flex flex-col items-start gap-2 rounded-lg px-5 py-4">
		...
		{#if !connected}
			<div class="text-fy-on-surface-subtle flex flex-row items-center gap-3 py-2 text-xs">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="24"
					height="24"
			...
			<div
				class="my-1 flex h-12 w-full flex-row justify-center overflow-hidden rounded bg-[#ffc439] shadow-lg"
			>
				<Paypal
					class="w-full"
					label="pay"
		...
		{:else if email}
			<div class="flex w-full flex-row justify-between py-2">
				<div class="flex flex-col text-sm">
					<!-- TODO: Retrieve the email from the PayPal SDK -->
					<span>Connected</span>
					<!-- <span class="font-semibold">{email}</span> -->
				</div>
				<!-- TODO: Make this button open the PayPal UI again -->
				<!-- <button type="button" on:click class="text-[#009CDE] mx-2 font-bold text-xs">
					Change
				</button> -->
			</div>
		{/if}
