<script>
	...
	import { PUBLIC_api_id } from '$lib-v4/env.js';
	import {
		initialize,
	...
	let multipleVariants = false;
	let iframeHeight = 0;
	let iframeDisplay = 'none';
	let ecsUrl = '';
	...
	async function initiateCheckoutByUrl(url) {
		...
		if (!skipCatalogApi) {
			...
			const resp = await fetch(`${PUBLIC_cf_server}/api/v1/domains-pdp?url=${url}`, {
				headers: {
