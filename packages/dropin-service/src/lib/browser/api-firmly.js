...
import { platformsToMakeCartHealthCheck } from './api-config.js';
...
function telemetryEnqueue(message) {
    ...
    if (Array.isArray(message)) {
        for (const iterator of message) {
            ...
            _telemetryQueue.push(Object.assign(iterator, { order: order, span_id: event_id, event_id: event_id }, headerProp));
        }
    } else {
        const order = ++_telemetryOrder;
        const event_id = getNextEventId();
        _telemetryQueue.push(Object.assign(message, { order: order, span_id: event_id, event_id: event_id }, headerProp));
    }
...
function telemetryVisaApiPerformance(durationMs, apiName, apiResponse, transactionId, additionalData) {
    const firmly = window.firmly;
    const headerProp = getHeaderProp();
...
async function browserFetch(url, options = defaultOptions) {
    ...
    if (res.status != 200) {
        try {
            ...
            console.error('Error enqueueing telemetry:', ex);
        }
    }
...
async function getRSAKey(jwkKey) {
    ...
    const publicKey = await CRYPTO.subtle.importKey('jwk', jwkKey, { name: 'RSA-OAEP', hash: 'SHA-1' }, true, ['encrypt']);
...
async function gcmEncrypt(inputData) {
    try {
        ...
        const encodedData = encoder.encode(typeof inputData === 'string' ? inputData : JSON.stringify(inputData));
        const { publicKey, version } = firmly.paymentRSAKey;

        // Generate a new AESGCM Key for encrypting the payload.
        const aesgcmKey = await CRYPTO.subtle.generateKey({ name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
        ...
        const b64Encoded = await CRYPTO.subtle.encrypt({ name: 'RSA-OAEP', hash: 'SHA-1' }, publicKey, cryptoConcat([rawAesgcmKey, iv, authTag]));
        ...
        console.error('Error during GCM encryption:', e);
        return null;
    }
}
...
async function rsaEncrypt(inputData) {
    try {
        ...
        const enc = await crypto.subtle.encrypt({ name: 'RSA-OAEP', hash: 'SHA-1' }, publicKey, Uint8Array.from(new TextEncoder().encode(plain)));
...
async function sessionGetAllActiveCarts(excludes = []) {
    ...
    return browserFetch(`${window.firmly.apiServer}/api/v1/carts/active?excludes=${excludes.join(',')}`, {
        ...headers,
        method: 'GET',
    });
}
...
const TAG_LIST = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'gclid', 'fbclid', 'shpxid'];
...
async function cartAddLineItem(sku, quantity, variantHandles = [], domain = undefined) {
    const headers = await getHeaders();
    const body = {
        variant_handles: variantHandles,
    };
    const res = await browserFetch(getDomainUrl(`cart/line-items/${sku}/quantity/${quantity}`, domain), {
        ...headers,
        method: 'POST',
        body: JSON.stringify(body),
    });
...
async function paypalCompleteOrder() {
    const headers = await getHeaders();
