...
function postSignIn(cookies) {
	const jData = JSON.stringify({ action: 'firmlySignIn', cookies: cookies });
	window.parent.postMessage(jData, '*');
}

function postQuantityUpdated(totalQuantity) {
