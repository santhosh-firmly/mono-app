<script context="module">
    import { PUBLIC_c2p_dpa_id, PUBLIC_c2p_initiator_id, PUBLIC_c2p_sdk_url } from '$lib/env.js';
    ...
    export async function visaUnlockStart(email) {
        ...
        if (consumerPresent) {
            const { maskedValidationChannel } = await performanceObserver(() => vSrc.initiateIdentityValidation(), 'initiateIdentityValidation');
    ...
    export async function unlockComplete(otpValue) {
        try {
            const res = await performanceObserver(() => visaUnlockComplete(otpValue), 'visaUnlockComplete');
            return {
                status: 200,
                data: res,
            };
