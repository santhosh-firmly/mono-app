<script context="module">
    import { PUBLIC_uc2p_sdk_url } from '$lib/env.js';
    ...
    async function performanceObserver(func, apiName, additionalData) {
        ...
        try {
            ...
            window.firmly.telemetryVisaApiPerformance?.(durationMs, apiName, response, uniqueTransactionId, additionalData);
        }
    }
    ...
    export async function initiateIdentityValidation() {
        const { maskedValidationChannel } = await performanceObserver(() => Vsb.initiateIdentityValidation(), 'initiateIdentityValidation');
            ...
            return {
                ...
                data: {
                    ...
                    payment_method_options: maskedCards.map((e) => ({
                        ...
                        type: 'CreditCard',
                        wallet: 'c2p',
                        provider: e.paymentCardDescriptor ?? 'visa',
                        ...
                        card_type: e.paymentCardDescriptor ?? 'VISA',
                        billing_info: {
                            ...
                            country: e.maskedBillingAddress.countryCode === 'US' ? 'United States' : e.maskedBillingAddress.countryCode,
    ...
    export async function unlockStart(email, validationData) {
        ...
        try {
            ...
            const res = await performanceObserver(() => initiateIdentityValidation(email), 'visaUnlockStart');
            if (res) {
                return {
                    status: 200,
                    data: res,
    ...
    export async function getVisaCardToken(cardId, cvv, rememberMe, additionalData) {
        try {
            ...
            const res = await performanceObserver(() => window.firmly.paymentC2PTokenize(cardId, cvv, null, jws), 'paymentC2PTokenize');
