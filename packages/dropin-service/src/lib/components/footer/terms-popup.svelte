<script>
    ...
    /**
     * @typedef {Object} termsPopupProps
     * @property {string} title - Title of the modal window
     * @property {boolean} isModalOpen - Controls whether of not the popup window is open
     */
    /**
     * @type {termsPopupProps}
     */
    let { title = '', isModalOpen = false } = $props();
...
<Modal bind:isModalOpen>
    <div class="flex h-full min-w-[300px] max-w-[720px] flex-col">
        <div class="flex flex-row items-center justify-between p-4 pb-3">
            <span class="text-lg font-bold leading-none"> {title} </span>
            <button onclick={() => (isModalOpen = false)} aria-label="Close terms and conditions">
                <svg class="ml-2" xmlns="http://www.w3.org/2000/svg" width={13} height={13} viewBox="0 0 13 13" fill="none">
                    <path
                        class="fill-gray-400"
        ...
        <div class="w-full shrink overflow-scroll p-4 pt-3 text-justify text-sm">
            {#if !termsAndConditions}
                <div transition:slide class="w-full text-center">
                    <svg aria-hidden="true" class="text-gray inline h-6 w-6 animate-spin fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
