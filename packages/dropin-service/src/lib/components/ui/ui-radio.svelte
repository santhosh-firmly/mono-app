...
<div class="flex items-center gap-3 {className}">
	<div class="relative flex h-5 w-5 items-center justify-center">
		<input
			...
			{disabled}
			class="
			border-border h-5 w-5 appearance-none rounded-full border-2
			{disabled ? 'cursor-not-allowed' : 'checked:border-primary checked:bg-background cursor-pointer'}"
		/>
		{#if isSelected}
			<div
				class="pointer-events-none absolute h-2.5 w-2.5 rounded-full"
