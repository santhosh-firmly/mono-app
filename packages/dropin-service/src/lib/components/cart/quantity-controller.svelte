...
<div class="mt-3 flex flex-row items-center">
    <Group stretch={false}>
        <div class="container col-span-2 flex flex-row items-center overflow-hidden rounded-lg" class:disabled>
            <button class="h-full w-8 p-1 px-2 text-xs font-bold hover:bg-gray-100 disabled:bg-gray-100" type="button" {disabled} on:click={() => updateItemQuantity(--quantity)}>
                {#if quantity <= 1}
            ...
            />
            <button class="text w-8 p-1 px-2 text-xs font-bold hover:bg-gray-100 disabled:bg-gray-100" type="button" {disabled} on:click={() => updateItemQuantity(++quantity)}>
                +
            </button>
        </div>
    </Group>
