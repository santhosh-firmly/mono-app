<script>
    ...
    import { debounce } from '$lib/browser/dash';
    ...
    let autocompleteSelectedIdx = $state(0);
...
<div class="relative z-[900] flex w-full justify-center">
    <div class="absolute top-1 w-full px-3">
        <Group>
            <div class="col-span-2 w-full rounded p-2 shadow-lg">
                {#each predictions as prediction, index}
                    <button
                        ...
                        onmouseover={() => (autocompleteSelectedIdx = index)}
                        onfocus={() => (autocompleteSelectedIdx = index)}
                        onclick={() => addressOnClick(prediction.id)}
                    >
                ...
                <button
                    type="button"
                    class="w-full rounded p-1 text-left text-xs text-fy-on-surface-subtle underline"
                    class:bg-fy-on-primary-subtle2={predictions.length === autocompleteSelectedIdx}
                    onmouseover={() => (autocompleteSelectedIdx = predictions.length)}
                    onfocus={() => (autocompleteSelectedIdx = predictions.length)}
                    onclick={() => {
                        dispatch('on-address-selected', { manual: true });
                    }}
                >
