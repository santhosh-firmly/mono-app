<script module>
	...
	import FastCheckout from './fast-checkout.svelte';

	const { Story } = defineMeta({
		title: 'Checkout/Fast Checkout',
		...
		argTypes: {
			isLoading: {
				control: 'boolean',
				description: 'Whether the buttons are in a loading/disabled state'
			},
			use: {
				control: 'check',
				options: ['paypal'],
				description: 'Array of payment methods to display'
			},
			onclick: {
				action: 'clicked',
				description: 'Callback when a payment method is clicked'
			}
		},
		args: {
			isLoading: false,
...
{#snippet template(args)}
	<div class="w-95 max-w-md p-4">
		<FastCheckout {...args} paypalButton={mockPaypalButton} />
	</div>
{/snippet}
