<script>
    ...
    let { icon, text, undoCallback, closeable = true, undoInProgress = false } = $props();
...
{#if showNotice}
    <div
        transition:fade
        class="col-span-2 flex items-center rounded-lg border-solid border-fy-primary bg-white p-4 text-gray-500 shadow-lg outline outline-1 outline-fy-on-primary-subtle2 dark:bg-gray-800 dark:text-gray-400"
    >
        {@render icon?.()}
        ...
        <div class="ml-auto flex flex-shrink-0 flex-row items-center gap-1 pl-2">
            ...
            {#if undoInProgress}
                <svg
                    aria-hidden="true"
                    class="text-gray-fy-on-action-subtle m-1.5 h-6 w-6 animate-spin fill-fy-on-action"
                    viewBox="0 0 100 100"
                    fill="none"
