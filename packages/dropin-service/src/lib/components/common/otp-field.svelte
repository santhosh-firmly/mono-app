<script>
    ...
    /**
     * @typedef {Object} OTPFieldProps
     * @property {number} otpLength - Length of the OTP
     * @property {boolean} autoSubmitWhenPasting - Auto submit when pasting
     * @property {boolean} autoSubmitWhenTyping - Auto submit when typing
     * @property {boolean} disabled - Disable the OTP field
     * @property {string} error - Error message
     * @property {function} onOTPComplete - Callback when the OTP is complete
     */
    /**
     * @type {OTPFieldProps}
     */
    let { otpLength = 6, autoSubmitWhenPasting = true, autoSubmitWhenTyping = true, disabled = false, error = '', onOTPComplete } = $props();
...
<form onsubmit={handleSubmit} class="flex flex-col gap-2 px-1 py-4">
    <div class="flex flex-row justify-center gap-2">
        ...
        {#each Array(otpLength) as { }, index (index)}
            <input
                ...
                {disabled}
                class="border-1 shadow-md {error ? 'border-red-600' : 'border-gray-300'} h-11 w-10 rounded-lg text-center disabled:bg-gray-100"
                type="text"
                ...
                ontextInput={onOTPTextInput}
            />
        {/each}
    </div>
    {#if error}
        <div data-testid="otp-error">
