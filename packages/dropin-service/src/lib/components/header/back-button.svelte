...
>
    ...
    <div class="relative flex h-full flex-row items-center gap-2 transition duration-300" class:-translate-x-6={!showBackButton}>
        {#if skeleton}
            ...
            ></div>
        {:else if largeLogo}
            <img class="inline h-4 transition duration-300 group-hover:-translate-x-1 {$isPrimaryDark ? 'grayscale invert' : ''}" src={largeLogo} alt="${displayName} logo" />
        {:else}
            {#if smallLogo}
                <img class="inline h-8 rounded-full transition duration-300 group-hover:-translate-x-1" src={smallLogo} alt="${displayName} logo" />
            {/if}
