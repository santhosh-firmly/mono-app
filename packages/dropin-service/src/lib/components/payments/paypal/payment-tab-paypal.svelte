<script>
	...
	import PaymentTab from '$lib/components/payments/payment-tab.svelte';

	/**
	 * @typedef {PaymentTabPaypalProps}
	 * @property {boolean} active - Whether the tab is active
	 * @property {boolean} disabled - Whether the tab is disabled
	 */
	/**
	 * @type {PaymentTabPaypalProps}
	 */
	let { active, disabled } = $props();
</script>

<PaymentTab on:click text="PayPal" {active} {disabled}>
