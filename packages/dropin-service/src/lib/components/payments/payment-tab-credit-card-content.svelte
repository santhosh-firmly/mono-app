<script>
    ...
    import { CARD_ICON_COMPONENTS, EmptyCardInfo, formatCardNumberRaw, getCardTypeByValue } from '$lib/browser/credit-card-helper.js';
    ...
    let {
        ...
        validateAndSubmitBillingAddress,
    ...
    export const validateCreditCard = async (event, showSchemaErrors) => {
        if (!event || !fieldset.contains(event.relatedTarget)) {
            try {
                ...
                if (parseInt(creditCard.year) <= now.getFullYear() && parseInt(month) < now.getMonth() + 1) {
    ...
    <div class="mb-3">
        <Group>
            {#each filteredCards as savedCard, index}
                <label class="col-span-2 flex w-full flex-row items-center gap-3 border-0 px-3 py-3" class:disabled-label={disabled} class:rounded-t-lg={index === 0}>
                    <input
                        name="credit-card-option"
                        class="text-fy-action disabled:bg-fy-on-primary-subtle"
                        type="radio"
                        {disabled}
                        value={savedCard.id}
                        bind:group={selectedCardOption}
                    />
                    <ExistingCreditCard type={savedCard.card_type?.toLowerCase()} number={savedCard.last_four} customArtUrl={savedCard.art} />
                </label>
            {/each}
            <label class="col-span-2 flex w-full flex-row items-center gap-3 rounded-b-lg border-0 px-3 py-3" class:disabled-label={disabled}>
                <input
                    name="credit-card-option"
                    class="text-fy-action disabled:bg-fy-on-primary-subtle"
                    type="radio"
                ...
                <div class="flex w-full items-center gap-3">
                    ...
                    <span class="text-sm font-bold text-fy-on-surface">Add New Card</span>
                </div>
            </label>
        </Group>
    </div>
...
{#if !filteredCards || filteredCards.length === 0 || selectedCardOption === NEW_CARD_OPTION}
    <fieldset bind:this={fieldset} onfocusout={validateCreditCard} transition:slide={{ duration: 150 }}>
        ...
        <Group>
            <div class="relative col-span-2 flex w-full flex-row justify-between rounded-t-lg">
                ...
                <div class="card-brand-icons absolute right-0 top-0 z-10 ml-4 flex h-full items-center pr-2">
                    {#if cardBrandDetected}
                        <span class="pr-2"><CardComponent></CardComponent></span>
                    {:else}
            ...
            <div class="relative flex w-full flex-col justify-center rounded-bl-lg">
                <input
                    class="w-full rounded-bl-lg border-0 placeholder:text-fy-on-primary-subtle disabled:bg-gray-100"
                    class:error
                    {disabled}
            ...
            <div class="relative flex w-full flex-row justify-center rounded-br-lg">
                <div class="flex w-full">
                    <input
                        class="w-full rounded-br-lg border-0 border-fy-on-primary-subtle placeholder:text-fy-on-primary-subtle disabled:bg-gray-100"
                        class:error
                        {disabled}
                ...
                <div class="absolute right-0 top-0 z-10 mr-3 flex h-full flex-col justify-center">
...
{#if selectedCardOption === NEW_CARD_OPTION}
    <div class="pt-5">
        <Checkbox
            ...
            on:change={({ detail }) => {
                isBillingSameShipping = detail.isChecked;
            }}
        />
    </div>
{/if}
