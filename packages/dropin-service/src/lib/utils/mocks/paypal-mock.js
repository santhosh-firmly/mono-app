...
export function setConfiguratorForPayPalMock(configurator) {
	configuratorInstance = configurator;
}

export function enablePayPalMock() {
	if (mockEnabled) return;
	mockEnabled = true;
	setupMockPayPalSDK();
}

export function disablePayPalMock() {
	mockEnabled = false;
...
function findBrowserFrameContainer() {
	if (typeof document === 'undefined') return null;

	const browserFrame = document.querySelector('[data-browser-frame]');
	if (browserFrame) return browserFrame;
...
function showOverlay() {
	if (typeof document === 'undefined' || overlayElement) return;

	overlayContainer = findBrowserFrameContainer();
	...
	overlayElement.id = 'paypal-mock-overlay';
	...
	overlayElement.innerHTML = `
		...
		">
			<svg xmlns="http://www.w3.org/2000/svg" width="120" height="32" viewBox="0 0 124 33" style="margin-bottom: 20px;">
			...
			<div style="
				...
				margin: 0 auto 16px;
			"></div>
			<p style="font-size: 16px; font-weight: 600; color: #1f2937; margin: 0 0 8px;">
				Simulating PayPal Flow
			</p>
			<p style="font-size: 14px; color: #6b7280; margin: 0;">
				In a real implementation, you would be redirected to PayPal to complete the payment.
			</p>
		</div>
		<style>
...
async function trackRequest(name, method = 'POST') {
	...
	await configuratorInstance.addRequest(name, {
		url: `paypal://${name.toLowerCase().replace(/\s+/g, '-')}`,
		method
	});
}

function setupMockPayPalSDK() {
		...
		Buttons: (config) => ({
			render: async (selector) => {
				...
				button.addEventListener('mouseenter', () => {
					button.style.background = '#f0b72f';
				});
				...
				button.onclick = async () => {
					try {
						...
						trackRequest('PayPal Create Order');

						const orderId = await config.createOrder();

						await new Promise((resolve) => setTimeout(resolve, 1500));

						trackRequest('PayPal User Approved');

						if (config.onApprove) {
							await config.onApprove({
								orderID: orderId,
								payerID: 'MOCK-PAYER-' + Date.now()
							});
						}
