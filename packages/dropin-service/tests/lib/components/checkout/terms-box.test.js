...
describe('TermsBox', () => {
	...
	describe('URL sanitization', () => {
		it('filters out anchors with null URLs', async () => {
			...
			expect(links[0].textContent.trim()).toBe('Valid');
		});

		it('filters out anchors with javascript protocol URLs', async () => {
			const user = userEvent.setup();
			const { container } = render(TermsBox, {
				props: {
					...defaultProps,
					anchors: [
						{ label: 'Valid', url: 'https://example.com' },
						{ label: 'XSS Attempt', url: 'javascript:alert(1)' }
					]
				}
			});
		...
		it('hides expand button when all anchors have invalid URLs', () => {
			const { container } = render(TermsBox, {
				props: {
					...defaultProps,
					anchors: [
						{ label: 'Bad', url: null },
						{ label: 'Also Bad', url: 'javascript:void(0)' }
					]
				}
			});
