<script context="module" lang="ts">
	...
	export async function visaUnlockStart(email: String) {
	...
	export async function isRecognized() {
		...
		if (recognized) {
			...
			return {
				...
				data: {
					recognized: true,
					payment_method_options: maskedCards.map((e: any) => ({
	...
	export async function unlockStart(email: String) {
		if (!vSrc) {
			await onSDKLoaded();
		}
		...
		try {
			...
			const res: any = await performanceObserver(() => visaUnlockStart(email), 'visaUnlockStart');
			if (res) {
				return {
					status: 200,
					data: res
	...
	export async function visaUnlockComplete(otpValue: String) {
	...
	export async function unlockComplete(otpValue: string) {
		try {
			const res: any = await performanceObserver(
	...
	async function visaCheckout(cardId: string, rememberMe: boolean, additionalData) {
