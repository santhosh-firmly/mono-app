<script>
	// @ts-nocheck
	import Countries from '$lib/components/common/countries.json';
...
<fieldset bind:this={fieldset} on:focusout={onFocusOut}>
	<Group>
		<div class="w-full col-span-2 relative flex flex-col justify-center rounded-t-lg">
			<input
				{disabled}
				bind:value={fullName}
		...
		<div
			class={!expanded && !hasPhone
				...
				: 'w-full col-span-2 relative flex flex-col justify-center overflow-visible'}
		>
			<input
				{disabled}
				class={!expanded && !hasPhone
					? 'border-0 w-full placeholder:text-fy-on-primary-subtle disabled:bg-gray-100 rounded-b-lg'
					: 'border-0 w-full placeholder:text-fy-on-primary-subtle disabled:bg-gray-100'}
			...
			<div class="absolute right-0 z-[2] text-xs font-semibold text-fy-on-primary-subtle">
				{#if !expanded}
					<button
						...
						id="extend"
						{disabled}
						class="rounded-full px-2 py-1 m-2 shadow"
						on:click={() => {
							expanded = true;
							shouldChangeTheFocus = false;
		...
		<div
			class="w-full col-span-2 relative flex flex-col justify-center"
			class:not-expanded={!expanded}
		>
			<input
				{disabled}
				class="border-0 w-full placeholder:text-fy-on-primary-subtle disabled:bg-gray-100"
		...
		<div class="w-full relative flex flex-col justify-center" class:not-expanded={!expanded}>
			<input
				{disabled}
				class="border-0 w-full placeholder:text-fy-on-primary-subtle disabled:bg-gray-100"
				bind:value={city}
		...
		<div class="w-full relative flex flex-col justify-center" class:not-expanded={!expanded}>
			<input
				{disabled}
				class="border-0 border-fy-on-primary-subtle w-full placeholder:text-fy-on-primary-subtle disabled:bg-gray-100"
		...
		<div
			class={expanded && !hasPhone
				? bottomFieldFormClass
				: 'w-full col-span-2 relative flex flex-col justify-center'}
			class:not-expanded={!expanded}
		>
			<input
				{disabled}
				class={expanded && !hasPhone
					? 'border-0 w-full col-span-2 placeholder:text-fy-on-primary-subtle disabled:bg-gray-100 rounded-b-lg'
					: 'border-0 w-full col-span-2 placeholder:text-fy-on-primary-subtle disabled:bg-gray-100'}
		...
		{#if hasPhone}
			<div class="w-full col-span-2 relative flex flex-col justify-center rounded-b-lg">
				<input
					bind:this={phoneEl}
					{disabled}
					class="border-0 w-full placeholder:text-fy-on-primary-subtle pl-10 rounded-b-lg disabled:bg-gray-100"
					data-testid="phone"
					...
					type="text"
				/>
				<span data-testid="flag" class="absolute left-0 p-3 z-[2]">
					{currentCountry?.flag || ''}
				</span>
			</div>
		{/if}
