<script>
	...
	import C2pIcon from '../common/svg/c2p-icon.svelte';
	import Group from './group.svelte';

	/**
	 * maskedCard from Visa to be rendered for the step up
	 */
	...
	onMount(() => {
		...
		if (authenticationMethods) {
			authenticationMethods = authenticationMethods.filter(
...
<div class="flex flex-col mt-4 max-h-[300px] transform scale-1/2">
	<div class="flex text-sm font-bold text-[#5e5e5e] items-center justify-center ml-4">
		Confirm it's you
	</div>
	<div class="flex text-sm font-medium items-center justify-center text-center mb-2 mx-4">
	...
	<div class="mt-4 ml-4 mr-4">
				...
				<Group>
					<div class="flex flex-col rounded-lg col-span-2">
						{#each authenticationMethods.filter( (o) => ['SMS_OTP', 'EMAIL_OTP'].includes(o.authenticationMethodType) ) as option, index (option.authenticationMethodType)}
							<div
								class="border-0 border-b-2 px-2 py-2"
								class:border-none={isLast(authenticationMethods, index)}
							>
								<label class="flex items-center px-3 py-2 gap-3 col-span-2 text-sm">
									<input
									...
									<div>
										{#if option.authenticationMethodType === 'SMS_OTP'}
											<span class="font-normal flex flex-col"
												>Text Code to <strong>{option?.authenticationCredentialReference}</strong
												></span
											>
										{/if}
		...
		<div class="flex items-center justify-center my-4">
			<button
				type="submit"
				class="relative font-medium w-full p-2 rounded text-sm flex flex-row {buttonClasses}"
			...
			>
				{#if currentStep === BASE_LOGIN_STEPS.PROCESSING_C2P_OTP_STEPUP}
					<span class="flex flex-row w-full justify-center items-center leading-6"
						><C2pIcon width={null} height={30} class="fill-white" /> Sending OTP...
					</span>
					<svg
						aria-hidden="true"
						class="absolute right-0 w-6 h-7 mr-2 animate-spin {spinnerClasses}"
						viewBox="0 0 100 101"
						fill="none"
