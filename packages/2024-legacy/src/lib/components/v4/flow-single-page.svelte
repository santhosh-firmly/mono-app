<script>
	...
	import { Required } from '$lib/browser/localization.js';
	...
	import { isEqual } from '$lib/browser/dash.js';
	import { writable } from 'svelte/store';
	import { postOrderPlaced, postQuantityUpdated, postSignIn } from '$lib/browser/cross.js';
	import LoginButton from './login-button.svelte';
	import Shoppay from './view-model/shoppay.svelte';
	import ClickToPay from './view-model/click-to-pay.svelte';
	import { BASE_LOGIN_STEPS, NEW_CARD_OPTION, NEW_SHIPPING_ADDRESS } from '$lib/constants.js';
	...
	import { getRandonPassword } from '$lib/browser/api-manager.js';
	...
	let paypalConnected;
	$: {
		paypalConnected = $cart?.payment_method?.type === 'PayPal';
	}
	...
	async function placeOrderPayPal() {
		...
		if (isEmailvalid && isShippingValid) {
			const completeOrderResponse = await window.firmly.paypalCompleteOrder();
	...
	async function placeOrderC2P(selectedCard, additionalData = {}) {
		const tokenizeResponse = await tokenizeC2P(selectedCard, additionalData);
	...
	async function onPlaceOrder(additionalData = {}) {
		try {
			...
			if (selectedPaymentMethod === 'CreditCard') {
				...
				orderPlaceResponse = await placeOrderPayPal(additionalData);
			} else {
				// Unexpected
			}
...
<div class="fixed bottom-0 top-0 left-0 right-0 md:bg-fy-primary z-[-1]"></div>
<div class="fixed w-1/2 bottom-0 top-0 right-0 md:bg-fy-background z-[-1] shadow"></div>
<div class="flex flex-row w-full justify-center">
	<form
		class="md:flex md:flex-row md:justify-around md:max-w-[1024px] w-full"
		on:submit={onPlaceOrder}
	>
		<section class="md:py-16 md:w-1/2 md:h-full flex flex-col items-center md:max-w-[412px]">
			<Overview
				on:back-click
				total={$cart?.total}
			...
			<div class="grow"></div>
			<div class="text-xs text-center max-md:hidden text-fy-on-primary-subtle">
				<FooterLinks />
			</div>

			<Notices bind:notices />
		</section>
		<section class="md:py-16 md:w-1/2 bg-fy-background md:max-w-[412px]">
			<!-- Skeleton -->
			{#if !$cart}
				<div class="flex flex-col gap-4 max-md:p-4 md:px-4">
					<div class="grid grid-cols-1 gap-4">
						<div class="flex justify-center">
							<div class="px-4 py-2 rounded-lg w-full h-8 bg-fy-on-primary-subtle2 animate-pulse" />
						</div>
						<div class="flex justify-center">
							<div class="px-4 py-2 rounded-lg w-full h-8 bg-fy-on-primary-subtle2 animate-pulse" />
						</div>
					</div>
					<div class="flex flex-row w-full text-fy-on-primary-subtle justify-center relative">
						<div
							class="absolute w-full h-full left-0 flex flex-col justify-center"
							style="z-index: -1;"
						>
					...
					<div>
						<div class="px-4 rounded w-40 h-4 my-1 bg-fy-on-primary-subtle2 animate-pulse" />
					</div>
					<div>
						<div class="px-4 rounded w-12 h-3 my-1 bg-fy-on-primary-subtle2 animate-pulse" />
						<div class="px-4 rounded-lg w-full h-10 my-1 bg-fy-on-primary-subtle2 animate-pulse" />
					</div>
					<div class="grid grid-cols-2 my-1 gap-1">
						<div
							class="px-4 rounded col-span-2 w-32 h-3 my-1 bg-fy-on-primary-subtle2 animate-pulse"
						/>

						<div
							class="px-4 rounded-lg w-full col-span-2 h-10 bg-fy-on-primary-subtle2 animate-pulse"
						/>
						<div
							class="px-4 rounded-lg w-full col-span-2 h-10 bg-fy-on-primary-subtle2 animate-pulse"
						/>
						<div
							class="px-4 rounded-lg w-full col-span-2 h-10 bg-fy-on-primary-subtle2 animate-pulse"
						/>
						<div class="px-4 rounded-lg w-full h-10 bg-fy-on-primary-subtle2 animate-pulse" />
						<div class="px-4 rounded-lg w-full h-10 bg-fy-on-primary-subtle2 animate-pulse" />
						<div
							class="px-4 rounded-lg w-full col-span-2 h-10 bg-fy-on-primary-subtle2 animate-pulse"
						/>
					...
					<div class="flex flex-col justify-center items-center mt-4">
						<div class="px-4 py-2 rounded-lg w-full h-16 bg-fy-on-primary-subtle2 animate-pulse" />
						<div class="px-4 rounded-lg w-64 h-4 my-3 bg-fy-on-primary-subtle2 animate-pulse" />
					</div>
				</div>
			{:else}
				<div class="flex flex-col max-md:p-4 md:px-4">
					{#if allowMerchantLogin || allowShopPay || allowPayPal}
						<div class="grid grid-cols-1 gap-4 py-2">
							...
							{#if allowShopPay}
								<div class="flex justify-center">
									<button
										type="button"
										class="bg-[#5a31f4] hover:bg-[#390ced] px-4 py-2 rounded text-white w-full flex flex-row items-center justify-center shadow"
										disabled={isC2PInProgress || placeOrderInProgress}
							...
							{#if allowPayPal}
								<div
									class="rounded w-full flex flex-row justify-center h-12 overflow-hidden shadow my-1 bg-[#ffc439]"
								>
									<Paypal
										class="w-full"
										label="paypal"
						...
						<div
							class="flex flex-row w-full text-fy-on-primary-subtle justify-center relative my-2"
						>
							<div class="absolute w-full h-full left-0 flex flex-col justify-center">
								<hr class="h-[1px] w-full" />
							</div>
							<span class="bg-fy-background px-4 text-sm z-10"> Or pay another way </span>
						</div>
					{/if}
					{#if collapsedStateShipping}
						<div transition:fadeSlide class="pt-2" class:pb-2={!collapsedStateShippingMethod}>
							<Group>
								<div
									class="flex flex-row justify-between items-center col-span-2 p-5 rounded-t-lg"
								...
								>
									{#if $cart.shipping_info}
										...
										<button
											type="button"
											class="text-blue-500 ml-5 text-sm rounded-full px-1"
									...
									{:else}
										<div class="w-full">
											<div class="w-48 bg-fy-on-primary-subtle2 rounded h-4 m-1 animate-pulse" />
											<hr />
											<div class="w-48 bg-fy-on-primary-subtle2 rounded h-4 m-1 animate-pulse" />
										...
										<div class="w-16 ml-5 bg-fy-on-primary-subtle2 rounded h-4 m-1 animate-pulse" />
									{/if}
								</div>
							</Group>
						</div>
					{:else}
						...
						{#if !savedAddresses || savedAddresses.length === 0 || selectedShippingAddress === NEW_SHIPPING_ADDRESS}
							<div class="py-2" transition:slide={{ duration: 150 }}>
								<div>
									<h3 class="text-sm py-1">Email</h3>
									<Group>
										<div class="w-full flex flex-col col-span-2 justify-center rounded-lg">
											<input
												class="border-0 w-full placeholder:text-fy-on-primary-subtle rounded-lg focus:z-[2] disabled:bg-gray-100"
									...
									{/if}
									{#if isC2PAvailable()}
										<div class="bg-[#F7F7F7] rounded-lg p-2 my-2">
											<span class="text-sm text-fy-on-surface-subtle leading-1 inline-block">
									...
									{#if marketingConsent && marketingConsent.ui_slot === 'UNDER_EMAIL_INPUT'}
										...
										>
											<span
												slot="title"
												class="text-xs pt-0.5 text-fy-on-surface-subtle font-normal"
											>
								...
								<div>
									<h3 class="text-sm py-1">Shipping Address</h3>
									<Address
					...
					{#if collapsedStateShippingMethod}
						...
						>
							<Group>
								<div
									class="flex flex-row items-center col-span-2 p-5"
								...
								>
									...
									{:else}
										<div class="w-full">
											<div class="w-56 bg-fy-on-primary-subtle2 rounded h-4 m-1 animate-pulse" />
											<div class="w-24 bg-fy-on-primary-subtle2 rounded h-4 m-1 animate-pulse" />
										</div>
										<div class="w-16 bg-fy-on-primary-subtle2 rounded h-4 m-1 animate-pulse" />
									{/if}
								</div>
							</Group>
						</div>
					{:else}
						<div transition:fadeSlide>
							<h3 class="text-sm py-1">Shipping Method</h3>
							...
							{:else}
								<div class="bg-[#F7F7F7] rounded-lg p-2 my-2">
									<span class="text-sm text-fy-on-surface-subtle leading-3">
					...
					{#if collapsedStatePayment}
						...
						<div class="pb-3" class:pt-2={!collapsedStateShippingMethod} transition:fadeSlide>
							<Group>
								<div
									class="flex flex-row items-center col-span-2 p-5 rounded-b-lg"
					...
					{#if marketingConsent && marketingConsent.ui_slot === 'ABOVE_PLACE_ORDER_BUTTON'}
						<Checkbox disabled={placeOrderInProgress} bind:isChecked={isMarketingConsentSigned}>
							<span slot="title" class="text-xs pt-0.5 text-fy-on-surface-subtle font-normal">
								{#each marketingConsent.parts as part}
									{#if part.type === 'text'}
										{part.content}
									{/if}
					...
					{#if place_order_error}
						<span class="text-xs text-fy-alert py-2">
							{place_order_error}
						</span>
					{/if}
					<Footer
						on:click={onPlaceOrder}
						disabled={shippingInfoInProgress || shippingMethodInProgress}
