...
<div>
	<Header {merchantInfo} {total} itemCount={order.line_items.length} on:back-click />
</div>

<div class="bg-fy-primary w-full px-3 pt-16 pb-5 gap-6 flex flex-col justify-center items-center">
	<div class="max-w-md">
		<div bind:this={animationContainer} class="p-4 flex flex-col justify-center items-center">
			<LottiePlayer
				width={56}
				height={56}
		...
		<div class="flex flex-col justify-center items-center gap-3">
			<span class="font-medium text-4xl text-fy-on-primary"> Thank you! </span>
			<span class="text-sm text-center font-normal text-fy-on-primary-subtle">
				Merchant will send the order confirmation
			</span>
		</div>
	</div>
	<div
		class="max-w-sm bg-fy-surface flex flex-col gap-4 px-8 py-6 w-full border rounded-lg shadow-sm divide-y"
	>
		<div class="flex flex-col justify-center items-center gap-3">
			...
			{/if}
			<div class="flex flex-row gap-2 text-fy-on-primary-subtle">
		...
		<div class="pt-4 grid grid-cols-2 gap-4">
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Merchant </span>
			<span class="text-sm text-end font-medium">
				{merchantName}
			</span>
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Order Number </span>
			<span class="text-sm text-end font-medium">
				{orderNumber}
			</span>
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Order Total </span>
			<span class=" text-sm text-end font-medium">
				{formatCurrency(total)}
			</span>
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Payment </span>
			...
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Email </span>
			<span class="text-sm text-end font-medium">
				{shippingInfo.email}
			</span>
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Shipping to </span>
			<span class="-primary text-sm text-end font-medium">
				{shippingInfo.name} · {shippingInfo.address} · {shippingInfo.phone}
			</span>
			<span class="text-fy-on-primary-subtle text-sm text-start font-normal"> Method </span>
			<span class="text-sm text-end font-medium">
				{shippingMethod}
			</span>
		</div>
	</div>
	<div class="text-fy-on-primary-subtle text-xs text-center pt-10">
		<FooterLinks />
	</div>
</div>
