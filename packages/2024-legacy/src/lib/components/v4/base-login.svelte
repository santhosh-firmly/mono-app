...
<div class="max-w-[360px]">
	...
	{#if currentStep === BASE_LOGIN_STEPS.WAITING_OTP || currentStep === BASE_LOGIN_STEPS.PROCESSING_OTP}
		<div class="flex items-center justify-between p-4 text-sm text-[#5e5e5e]">
			...
			{#if !hideChangeButton}
				<button
					class="font-bold text-xs p-3 {textClasses} cursor-pointer rounded-full hover:underline hover:underline-offset-4"
	...
	{#if currentStep === BASE_LOGIN_STEPS.WAITING_EMAIL || currentStep === BASE_LOGIN_STEPS.PROCESSING_EMAIL}
		<form class="mt-4 ml-4 mr-4" on:submit={onEmailSet}>
			...
			<div class="flex items-center justify-center mt-4">
				<input
					...
					type="text"
					class="border border-[#e0e0e0] shadow-[0_2px_6px_0px_rgba(136,136,136,0.25)] rounded-lg w-full h-[46px] text-sm {emailError
						? 'border-1 border-fy-alert'
						: ''} "
					placeholder="Email"
					autocomplete="email"
					bind:value={email}
			...
			<div class="flex items-center justify-center mt-4">
				<button
					type="submit"
					class="relative font-medium w-full p-2 rounded text-sm flex flex-row items-center {buttonClasses}"
					disabled={currentStep === BASE_LOGIN_STEPS.PROCESSING_EMAIL}
			...
			<div class="flex my-3.5 pl-2 pr-2">
				<span class="text-xs text-center text-[#5e5e5e]"
