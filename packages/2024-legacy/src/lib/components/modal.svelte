...
<script>
	...
	import { MOBILE, NA, setContentStyle } from '$lib/browser/layout.js';
	import { postCheckoutClosed, postOrderPlaced, postRefresh } from '$lib/browser/cross.js';
	import { sSignedInOnThisSession } from '$lib/browser/api-manager.js';
...
{#if contentComponent}
	<div
		id="bg"
		class="fixed z-[2000] top-0 left-0 flex flex-col justify-center w-screen h-full bg-black/50 scroll-smooth"
	...
	>
		<div bind:this={wrap} id="wrap" class="max-h-full max-w-full">
			<div
				class="fixed overflow-y-auto scroll-smooth bg-default h-full w-full"
