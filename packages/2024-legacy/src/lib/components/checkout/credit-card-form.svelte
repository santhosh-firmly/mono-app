<script lang="ts">
	...
	import { getAddress, searchAddress } from '$lib/browser/api-manager';
...
<form on:submit={handleSubmit}>
	...
	<div class="flex flex-col m-1.5 my-6">
		<div>
		...
		{#if !$form.sameAsShipping}
			<div>
				<form on:submit={handleSubmit} class="flex flex-col gap-2 pt-2 pb-2">
					<div class="flex text-lg flex-row h-10 mt-3 font-bold">Billing Address</div>
	...
	<div class="flex items-end justify-end m-3 my-6">
		<button
			type="submit"
			disabled={disabled || !canPlaceOrder || showSpinner}
			class="w-full text-center focus:ring-4 focus:outline-none items-center justify-center px-5 py-2 text-white focus:ring-blue-300 rounded-3xl hover:bg-blue-800 font-semibold text-normal bg-[#4c7cec] disabled:bg-gray-400 flex flex-row"
		>
			{#if showSpinner}
				<ApiProgressIcon_2 />
			{:else}
