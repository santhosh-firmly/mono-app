<script>
	 ...
	 * }}
	 */
	let { currentAppId, destinationAccess, merchantAccess = [] } = $props();

	let searchQuery = $state('');
	let popoverOpen = $state(false);
	let highlightedIndex = $state(-1);

	// Get current destination's display name
	let currentDestination = $derived(destinationAccess.find((d) => d.appId === currentAppId));
	let currentDisplayName = $derived(currentDestination?.displayName || currentAppId);

	// Total dashboard count for search threshold
	let totalDashboards = $derived(destinationAccess.length + merchantAccess.length);
		...
		<div class="max-h-80 overflow-y-auto py-1">
			...
			{#if filteredDestinations.length > 0}
				...
				{#each filteredDestinations as destination, index (destination.appId)}
					{@const isCurrentAppId = destination.appId === currentAppId}
					{@const isHighlighted = index === highlightedIndex}
					<a
						href={`/destination/${destination.appId}`}
						class="flex w-full items-center gap-2 px-2 py-1.5 text-sm transition-colors hover:bg-accent hover:text-accent-foreground {isCurrentAppId
							? 'bg-primary/5'
					...
					>
						...
						<div class="flex flex-col">
							<span class="max-w-36 truncate" class:font-medium={isCurrentAppId}>
