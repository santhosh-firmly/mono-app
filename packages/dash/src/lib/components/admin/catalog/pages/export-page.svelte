<script>
	...
	async function triggerExports() {
		...
		try {
			await onTriggerExport(domainsToExport, {
				onProgress: (index, status, jobId, error) => {
					exportProgress[index].status = status;
					if (jobId) exportProgress[index].jobId = jobId;
					if (error) exportProgress[index].error = error;
	...
	async function viewExportHistory(domain, countryCode) {
		...
		try {
			exportHistory = await onViewHistory(domain, countryCode);
		} catch (e) {
