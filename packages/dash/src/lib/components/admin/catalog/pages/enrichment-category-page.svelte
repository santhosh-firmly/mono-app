...
<div class="flex flex-col gap-6 p-4 sm:px-6">
	...
	<div>
		<div class="flex items-center gap-2 text-sm text-muted-foreground mb-2">
			<a href={breadcrumbHref} class="text-primary hover:underline">Enrichment</a>
			<span>/</span>
			<span>{categoryName}</span>
		</div>
		<PageHeader
			title={categoryName}
			description={category?.category?.description || 'Category details'}
		/>
	</div>
		...
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
			...
			<Card.Root>
				<Card.Content class="p-4">
					...
					<div class="text-2xl font-bold">{category?.domains?.length ?? 0}</div>
				</Card.Content>
			</Card.Root>
			<Card.Root>
				<Card.Content class="p-4">
			...
			{#if category?.category?.aliases?.length}
				<Card.Root>
					<Card.Content class="p-4">
						<div class="text-xs text-muted-foreground uppercase mb-1">Aliases</div>
						<div class="text-2xl font-bold">
							{category.category.aliases.length}
						</div>
					</Card.Content>
				</Card.Root>
			{/if}
				...
				<Card.Root>
					...
					<Card.Content>
						<div class="flex flex-col gap-2 max-h-[300px] overflow-y-auto">
							{#each category.domains as item (item.domain)}
								{@const maxMappings = Math.max(
									...category.domains.map((d) => d.mappingCount)
								)}
