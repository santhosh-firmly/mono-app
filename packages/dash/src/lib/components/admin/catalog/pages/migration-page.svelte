<script>
	...
	let completedItems = $derived(stats.filter((s) => s.status === 'migrated'));
	let errorItems = $derived(stats.filter((s) => s.status === 'error'));

	async function migrateDomain(domain, dryRun = false) {
		migratingDomain = domain;
		try {
			const result = await onMigrateDomain(domain, dryRun);
	...
	async function migrateAll() {
		migratingAll = true;
		try {
			await onMigrateAll(pendingItems);
		} finally {
			migratingAll = false;
		}
	}
		...
		<div class="flex items-center gap-4">
			...
			{#if pendingItems.length > 0}
				<Button onclick={migrateAll} disabled={migratingAll}>
					...
					{/if}
					Migrate All ({pendingItems.length})
				</Button>
			{/if}
		</div>
	</div>
	...
	{:else}
		...
		{#if pendingItems.length > 0}
			<section>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
					{#each pendingItems as stat (stat.domain)}
						<Card.Root>
							<Card.Content class="p-4">
		...
		{#if errorItems.length > 0}
			<section>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
					{#each errorItems as stat (stat.domain)}
		...
		{#if completedItems.length > 0}
			<section>
				<div class="flex items-center gap-2 mb-4">
					...
					<Badge>{completedItems.length}</Badge>
				</div>
				<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
					{#each completedItems as stat (stat.domain)}
