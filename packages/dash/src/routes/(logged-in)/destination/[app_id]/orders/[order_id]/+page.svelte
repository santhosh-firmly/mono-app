<script>
	...
	async function fetchOrder() {
		...
		try {
			const response = await adminFetch(`/destination/${appId}/api/orders/${orderId}`);
	...
	function navigateToOrder(orderNumber) {
		if (orderNumber) {
			goto(`/destination/${appId}/orders/${orderNumber}`);
		}
	}
</script>
...
<div class="space-y-6">
	...
	{:else if error}
		<Card.Root>
			<Card.Content class="py-12 text-center">
				...
				<Button variant="outline" onclick={() => goto(`/destination/${appId}/orders`)}>
	...
	{:else if order}
		...
		<OrderNavigation
			...
			onBack={() => goto(`/destination/${appId}/orders`)}
		...
		<div class="grid gap-6 lg:grid-cols-3">
			...
			<!-- Right column - Merchant, Customer, shipping, and payment info -->
			<div class="space-y-6">
				<!-- Merchant information (instead of Partner for destination view) -->
				<PartnerInfoCard name={merchantName} title="Merchant" />
