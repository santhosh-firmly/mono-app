...
export async function load({ platform }) {
    ...
    const orderSumStmt2 = platform.env.reporting.prepare('SELECT coalesce(SUM(order_total), 0) FROM orders WHERE created_dt > ?1 AND created_dt < ?2');
    ...
    const orderByMerchantStmt2 = platform.env.reporting.prepare('SELECT shop_id, SUM(order_total) as net_sales, count(*) as total_orders, avg(order_total) as aov FROM orders WHERE created_dt > ?1 AND created_dt < ?2 GROUP BY shop_id');
    ...
    const orderByDestinationStmt2 = platform.env.reporting.prepare('SELECT app_id, SUM(order_total) as net_sales, count(*) as total_orders, avg(order_total) as aov FROM orders WHERE created_dt > ?1 AND created_dt < ?2 GROUP BY app_id');
    ...
    const [wtd, mtd, ytd, lytd, ordersByMerchantWtd, ordersByMerchantMtd, ordersByMerchantYtd, ordersByMerchantLytd, ordersByDestinationWtd, ordersByDestinationMtd, ordersByDestinationYtd, ordersByDestinationLytd, destinations, merchants] = await Promise.all([
        ...
        orderByMerchantStmt2.bind(lastYearStartOfYear.toISOString(), lastYearEndOfYear.toISOString()).all(),
        ...
        orderByDestinationStmt2.bind(lastYearStartOfYear.toISOString(), lastYearEndOfYear.toISOString()).all(),
        destinationsStmt.all(),
        merchantsStmt.all(),
    ]);

    return {
        wtd, mtd, ytd, lytd, ordersByMerchantWtd, ordersByMerchantMtd, ordersByMerchantYtd, ordersByMerchantLytd,
        ordersByDestinationWtd, ordersByDestinationMtd, ordersByDestinationYtd, ordersByDestinationLytd, destinations,
        merchants
    };
}
