<script>
	...
	async function loadData() {
		...
		try {
			const [dashboardData, workflowData] = await Promise.all([
				catalogApi.domains.get(domain),
				catalogApi.workflows.list(domain, { limit: 10 })
			]);
			stats = dashboardData;
			workflows = workflowData.workflows || [];

			// Try to load latest export
			try {
				latestExport = await catalogApi.exports.getLatest(domain);
			} catch {
				latestExport = null;
			}
		} catch (e) {
	...
	async function openConfig() {
		...
		try {
			config = await catalogApi.domains.getConfig(domain);
		} catch (e) {
	...
	async function saveConfig() {
		...
		savingConfig = true;
		try {
			await catalogApi.domains.updateConfig(domain, config);
	...
	async function viewWorkflow(workflow) {
		...
		try {
			const result = await catalogApi.workflows.get(domain, workflow.workflow_id);
	...
	function downloadExport(filename) {
		...
		const url = catalogApi.exports.getFileUrl(domain, latestExport.workflowId, filename);
