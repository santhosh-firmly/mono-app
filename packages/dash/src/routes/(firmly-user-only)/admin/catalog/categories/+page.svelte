<script>
	...
	async function loadCategories() {
		...
		error = null;
		try {
			stats = await catalogApi.categories.getAll();
		} catch (e) {
		...
		<div class="flex gap-6 flex-wrap">
			<div class="flex flex-col gap-1">
				<span class="text-xs text-muted-foreground uppercase tracking-wide"
					>Total Domains</span
				>
				<span class="text-2xl font-bold">{stats.totalDomains}</span>
			</div>
			<div class="flex flex-col gap-1">
				<span class="text-xs text-muted-foreground uppercase tracking-wide"
					>Total Products</span
				>
				<span class="text-2xl font-bold">{stats.totalProducts?.toLocaleString()}</span>
			</div>
			<div class="flex flex-col gap-1">
				<span class="text-xs text-muted-foreground uppercase tracking-wide">Categories</span
				>
				<span class="text-2xl font-bold">{stats.categories?.length}</span>
			</div>
		</div>

		{#if stats.categories && stats.categories.length > 0}
			<div class="flex flex-col gap-3">
				{#each stats.categories as category (category.category)}
